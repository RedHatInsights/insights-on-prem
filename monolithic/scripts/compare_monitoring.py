...
try:
    ...
    print("Install with:")
    print("  UV_NATIVE_TLS=1 uv pip install --python venv/bin/python3 matplotlib pandas")
    sys.exit(1)
...
def discover_runs(base_dir):
    ...
    for entry in sorted(os.listdir(base_dir)):
        ...
        elif entry.startswith('monitoring_') and os.path.isdir(os.path.join(base_dir, entry)):
...
def plot_container_comparison(runs, container, output_path, dpi=150):
    ...
    fig.suptitle(f'{container} — Comparison Across Runs', fontsize=14, fontweight='bold')
    ...
    fig.legend(lines_for_legend, labels_for_legend,
               loc='lower center', ncol=min(4, len(runs)),
...
def main():
    parser = argparse.ArgumentParser(description='Compare monitoring data across test runs')
    ...
    if args.only:
        runs = [(l, p) for l, p in runs if l in args.only]

    if not runs:
        print("ERROR: No monitoring runs found.")
        sys.exit(1)

    print(f"Found {len(runs)} runs: {', '.join(l for l, _ in runs)}\n")
